...
import { useUniversalSearch, TYPE_META, type SearchResult } from '../hooks/useUniversalSearch';
...
function ExpandedSection({
  ...
  data: { loading: boolean; items: SearchResult[] };
  expanded: ExpandedChildren;
  alreadySelected: Set<string>;
  onSelect: (item: SearchResult) => void;
  onExpand: (type: string, id: string) => void;
  depth: number;
}) {
  const pl = depth === 1 ? 'pl-8' : 'pl-14';
  const textSize = depth === 1 ? 'text-xs' : 'text-[11px]';

  if (data.loading) return <div className={`${pl} pr-3 py-2 text-[10px] text-gray-400`}>Loading...</div>;
  if (data.items.length === 0) return <div className={`${pl} pr-3 py-2 text-[10px] text-gray-400`}>No items</div>;

  return (
    <div className={depth === 1 ? 'bg-gray-50/50 dark:bg-gray-900/30' : ''}>
      {data.items.map(child => {
        ...
        return (
          <div key={child.id}>
            <div
              className={`flex items-center gap-2 ${pl} pr-3 py-1.5 cursor-pointer transition-colors ${
                childSelected
                  ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
                  : 'hover:bg-gray-100 dark:hover:bg-gray-700/30 text-gray-700 dark:text-gray-300'
              }`}
              onClick={() => !childSelected && onSelect(child)}
            >
              {childCanExpand ? (
                <button
                  onClick={e => { e.stopPropagation(); onExpand(child.type, child.id); }}
              ...
              <span className={`${textSize} flex-1 truncate ${child.done ? 'line-through opacity-50' : ''}`}>
                {child.title}
              </span>
              {childSelected && (
            ...
            {childIsExpanded && expanded[childExpandKey] && depth < 2 && (
              <ExpandedSection
                data={expanded[childExpandKey]}
                expanded={expanded}
                alreadySelected={alreadySelected}
                onSelect={onSelect}
                onExpand={onExpand}
